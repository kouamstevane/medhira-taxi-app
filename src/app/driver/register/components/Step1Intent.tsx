...
interface Step1IntentProps {
  ...
  autoAdvanceOnMount?: boolean;
}

export default function Step1Intent({ onNext, onGoogleSignIn, initialData, loading, sendVerificationCode, verifyCode, onVerified, emailPreVerified = false, autoAdvanceOnMount = true }: Step1IntentProps) {
  ...
  useEffect(() => {
    if (autoAdvanceOnMount && emailPreVerified && !autoAdvancedRef.current && onVerified) {
      autoAdvancedRef.current = true;
      onVerified();
    }
  }, [autoAdvanceOnMount, emailPreVerified, onVerified]);
  ...
  if (verificationPhase && !codeVerified && sendVerificationCode && verifyCode) {
    ...
    return (
      <div className={cn(driverSectionCardClassName, 'space-y-6')} data-testid="otp-verification-screen">
        <div className="text-center mb-8">
          <div className="mx-auto w-16 h-16 bg-[#f29200]/20 rounded-full flex items-center justify-center mb-4">
            <span className="text-3xl">✉️</span>
          </div>
          <h2 className="text-2xl font-bold text-white">{t('driver.verifyYourEmail')}</h2>
          <p className="text-[#9CA3AF] mt-2">{t('driver.enter6DigitCode')}</p>
        </div>
        <OTPInput
          email={email}
          onVerify={verifyCode}
  ...
  return (
    <div className="space-y-6" data-testid="step1-registration-form">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-white">{t('driver.earnWithMedjira')}</h2>
        <p className="text-[#9CA3AF] mt-2">{t('driver.earnWithMedjiraSubtitle')}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          className={cn(
            ...
            'relative h-14 justify-center rounded-2xl border-white/10 bg-white/[0.03] px-5 text-[15px] font-semibold text-white hover:bg-white/[0.06]',
          )}
        >
          <span
            data-testid="google-signin-icon"
          ...
          <span className="mx-auto">{t('driver.continueWithGoogle')}</span>
        </button>
        ...
        <div className="relative py-4">
          ...
          <div className="relative flex justify-center">
            <span className="bg-[#171717] px-4 text-sm text-[#9CA3AF]">{t('driver.orManualRegistration')}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          <button
            ...
            className={cn(driverPrimaryButtonClassName, 'mt-6')}
          >
            {loading ? (
              <span className="animate-pulse">{t('common.loading')}</span>
            ) : (
              t('driver.continueRegistration')
            )}
          </button>
        </form>
      </div>
