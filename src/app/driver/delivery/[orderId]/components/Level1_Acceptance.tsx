    ...
    <div className="min-h-screen bg-background text-slate-100 flex flex-col items-center justify-center p-4">
      <div className="glass-card rounded-2xl border border-white/10 p-6 w-full max-w-sm space-y-5">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-white mt-2">{t('newDeliveryOrder')}</h2>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm text-slate-300">
            ...
            <span>{order.orderNumber} — {t('deliveryItemsCount', { count: order.orderItems.length })}</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-300">
            <MaterialIcon name="location_on" className="text-[18px] text-slate-500" />
            <span>{t('deliveryToNeighbourhood', { neighbourhood: order.clientNeighbourhood })}</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-300">
            <MaterialIcon name="payments" className="text-[18px] text-slate-500" />
            <span>{t('driverEarningsLabel')} <span className="text-primary font-bold">{order.driverEarnings?.toFixed(2)} $</span></span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {refusing ? <div className="w-4 h-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin mx-auto" /> : t('decline')}
          </button>
          <button
          ...
          >
            {accepting ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto" /> : t('acceptDeliveryBtn')}
          </button>
        </div>
      </div>
    </div>
