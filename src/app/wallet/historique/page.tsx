...
export default function WalletHistoriquePage() {
  ...
  const filters: { value: FilterType; label: string }[] = [
    { value: 'all',        label: t('wallet.filters.all') },
    { value: 'deposit',    label: t('wallet.filters.deposit') },
    { value: 'payment',    label: t('wallet.filters.payment') },
    { value: 'withdrawal', label: t('wallet.filters.withdrawal') },
  ];
  ...
  const formatDate = (date: Date) =>
    date.toLocaleDateString(dateLocale, { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });

  const groupByMonth = (txs: Transaction[]) => {
    const groups: Record<string, Transaction[]> = {};
    txs.forEach(t => {
      const key = t.date.toLocaleDateString(dateLocale, { month: 'long', year: 'numeric' });
  ...
  return (
    <div className="min-h-screen bg-background pb-24 max-w-[430px] mx-auto">
      ...
      <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-white/5 px-4 py-4 flex items-center gap-3">
        <button onClick={() => router.back()} aria-label={t('common.back')} className="p-2 rounded-full hover:bg-white/5 transition min-w-[44px] min-h-[44px] flex items-center justify-center">
          <MaterialIcon name="arrow_back" size="md" className="text-white" />
        </button>
        <h1 className="text-xl font-bold text-white flex-1">{t('wallet.historyTitle')}</h1>
        <Link
          href="/wallet/recharger"
          className="flex items-center gap-2 px-4 py-2 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 active:scale-95 transition-all min-h-[44px]"
        >
      ...
      <main className="px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass-card p-4 rounded-2xl border border-white/5">
            <p className="text-xs text-slate-400 mb-1">{t('wallet.totalCredits')}</p>
            <p className="text-xl font-bold text-green-400">{formatCurrencyWithCode(totalCredits)}</p>
          </div>
          <div className="glass-card p-4 rounded-2xl border border-white/5">
            <p className="text-xs text-slate-400 mb-1">{t('wallet.totalDebits')}</p>
        ...
        {loading ? (
          ...
          <div className="glass-card rounded-2xl p-10 text-center border border-white/5">
            ...
            <p className="text-white font-bold mb-1">{t('wallet.noTransactions')}</p>
            <p className="text-slate-400 text-sm">{t('wallet.noTransactionsInCategory')}</p>
          </div>
        ) : (
          <div className="space-y-6">
            {Object.entries(grouped).map(([month, txs]) => (
              <div key={month}>
                ...
                <div className="space-y-2">
                  {txs.map(tItem => {
                    const style = TRANSACTION_ICONS[tItem.type] ?? TRANSACTION_ICONS.payment;
                    const isCredit = tItem.type === 'deposit' || tItem.type === 'refund';
                    return (
                      <div key={tItem.id} className="glass-card p-4 rounded-2xl border border-white/5 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-white truncate">
                            {tItem.method || (isCredit ? t('wallet.deposit') : t('wallet.payment'))}
                          </p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <p className="text-xs text-slate-500">{formatDate(tItem.date)}</p>
                        ...
                        <span className={`font-bold text-sm shrink-0 ${isCredit ? 'text-green-400' : 'text-red-400'}`}>
                          {isCredit ? '+' : '-'}{formatCurrencyWithCode(tItem.netAmount ?? tItem.amount)}
                        </span>
                      </div>
                    );
                  })}
            ...
            {hasMore && (
              <button
                ...
                className="w-full min-h-[44px] py-3 border border-primary/40 text-primary font-bold rounded-2xl hover:bg-primary/5 transition disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loadingMore
                  ? <><MaterialIcon name="progress_activity" size="sm" className="animate-spin" /> {t('wallet.loadingMore')}</>
                  : t('wallet.loadMore')
                }
              </button>
            )}
