...
export function CustomerMenuItemDetails({
  ...
  const formatNutritionValue = (label: string, value: number): string => {
    if (label === 'calories') {
      return `${value} kcal`;
    }

    if (label === 'proteinGrams') {
      return t('proteinGrams', { count: value });
    }

    if (label === 'carbsGrams') {
      return t('carbsGrams', { count: value });
    }

    if (label === 'fatGrams') {
      return t('fatGrams', { count: value });
    }

    return t('saltGrams', { count: value });
  };
  ...
  }, [isOpen, item.id, restaurant.id, t]);
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/70 p-0 md:items-center md:p-4">
      ...
      <section
        aria-label={t('detailsNamed', { name: item.name })}
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">{t('menu')}</p>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {details.allergens.length > 0 ? (
              <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <h3 className="text-sm font-semibold text-white">{t('allergens')}</h3>
            ...
            {nutritionEntries.length > 0 ? (
              <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <h3 className="text-sm font-semibold text-white">{t('nutritionValues')}</h3>
                <div className="mt-3 flex flex-wrap gap-2">
            ...
            <section>
              <h3 className="text-lg font-bold text-white">{t('customization')}</h3>
